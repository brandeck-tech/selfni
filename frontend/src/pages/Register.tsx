  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0f172a', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'Arial' }}>
      <div style={{ background:'#1e293b', padding:'2rem', borderRadius:'16px', width:'90%', maxWidth:'400px' }}>
        ...
        <p style={{ color:'#94a3b8', textAlign:'center', marginBottom:'2rem' }}>إنشاء حساب جديد</p>
        ...
        <input placeholder="الاسم" value={form.username} onChange={e => setForm({...form, username: e.target.value})}
        ...
        <input placeholder="الإيميل" type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})}
          style={{ width:'100%', padding:'0.75rem', borderRadius:'8px', border:'1px solid #334155', background:'#0f172a', color:'white', marginBottom:'1rem', boxSizing:'border-box', textAlign:'right' }} />
        <input placeholder="كلمة السر" type="password" value={form.password} onChange={e => setForm({...form, password: e.target.value})}
        ...
        <p style={{ color:'#94a3b8', textAlign:'center', marginTop:'1rem' }}>
          عندك حساب؟{' '}
          <span onClick={onSwitch} style={{ color:'#38bdf8', cursor:'pointer' }}>سجل دخول</span>
        </p>
      </div>
    </div>
  );
