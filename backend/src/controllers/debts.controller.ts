...
export const addDebt = async (req: AuthRequest, res: Response) => {
  const { person_name, phone, amount, type, description, due_date } = req.body;
  if (!person_name || !amount || !type) return res.status(400).json({ status: 'error', message: 'الاسم والمبلغ والنوع مطلوبين' });
  try {
    const result = await query(
      'INSERT INTO debts (user_id, person_name, phone, amount, type, description, due_date) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',
      [req.user!.id, person_name, phone || null, amount, type, description, due_date]
    );
    return res.status(201).json({ status: 'success', debt: result.rows[0] });
  } catch (e: any) {
    // لو العمود phone مش موجود في الداتابيز، نضيفه
    if (e.code === '42703') {
      await query('ALTER TABLE debts ADD COLUMN IF NOT EXISTS phone VARCHAR(20)');
      const result = await query(
        'INSERT INTO debts (user_id, person_name, phone, amount, type, description, due_date) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',
        [req.user!.id, person_name, phone || null, amount, type, description, due_date]
      );
...
export const markAsPaid = async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  try {
    const result = await query('UPDATE debts SET is_paid=true, paid_at=NOW() WHERE id=$1 AND user_id=$2 RETURNING *', [id, req.user!.id]);
    if (result.rows.length === 0) return res.status(404).json({ status: 'error', message: 'الدين مش موجود' });
