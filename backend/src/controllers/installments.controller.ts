...
export const getInstallments = async (req: AuthRequest, res: Response) => {
...
export const createInstallments = async (req: AuthRequest, res: Response) => {
  ...
  if (!count || count < 2) return res.status(400).json({ status: 'error', message: 'عدد الأقساط لازم يكون 2 على الأقل' });
  try {
    const debtCheck = await query('SELECT * FROM debts WHERE id=$1 AND user_id=$2', [debtId, req.user!.id]);
    if (debtCheck.rows.length === 0) return res.status(404).json({ status: 'error', message: 'مش موجود' });
